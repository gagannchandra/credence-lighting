...
export default function BackButton({ fallback }) {
  ...
  return (
    <motion.button
      ...
      whileTap={{ scale: 0.96 }}
      onClick={handleBack}
      className="fixed top-5 left-5 z-[100] flex items-center gap-2 rounded-button bg-transparent/50 px-4 py-2 text-white text-sm font-semibold tracking-[0.2em] uppercase shadow-2xl backdrop-blur-lg border border-white/10 hover:bg-transparent/70 hover:border-white/20 transition duration-300"
    >
      <ArrowLeft size={16} />
      <span>Back</span>
    </motion.button>
